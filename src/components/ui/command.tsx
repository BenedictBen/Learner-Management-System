...
>(({ className, ...props }, ref) => {
  return (
    <CommandPrimitive
      ref={ref}
      className={cn(
        "flex w-full bg-casbGreyPrimary flex-col border-b-2 overflow-hidden rounded-sm text-gra-400 dark:bg-white dark:text-black",
      className
      )}
      {...props}
    />
...
// CommandInput component adjustments
const CommandInput = React.forwardRef<
  React.ElementRef<typeof CommandPrimitive.Input>,
  React.ComponentPropsWithoutRef<typeof CommandPrimitive.Input>
>(({ className, ...props }, ref) => {
  return (
    <div className="flex w-full items-center border-b px-3 h-10 md:h-8 min-w-0" cmdk-input-wrapper="">
      <Image
        alt="search-blue"
        src="/search-blue.png"
      ...
      <CommandPrimitive.Input
        ref={ref}
        className={cn(
          "flex-1 min-w-0 w-full h-full rounded-md bg-transparent text-sm md:text-base outline-none box-border",
        "placeholder:text-casbGreyPrimary disabled:cursor-not-allowed disabled:opacity-50 dark:placeholder:text-white",
        className
        )}
        {...props}
